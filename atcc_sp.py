...
class VehicleTracking(object):
    def __init__(
        ...
        if input_path.startswith("inputs"):
            ...
            self.input_path = "rtsp://admin:admin@1234@192.168.1.65/1"
        elif input_path == "kurnul":
            self.camera_id = input_path
            self.input_path = "rtsp://admin:admin@1234@192.168.1.64/1"
        else:
        ...
        if self.write_db:
            if self.camera_id == "hyderabad":
                self.sql_insert = """INSERT INTO Towards_Hyderabad(AVC_ID,AVC_Class,Date,Time,Lane,Axle_Configuration) VALUES(?,?,?,?,?,?)"""
            elif self.camera_id == "kurnul":
                self.sql_insert = """INSERT INTO Towards_Kernool(AVC_ID,AVC_Class,Date,Time,Lane,Axle_Configuration) VALUES(?,?,?,?,?,?)"""

            try:
                self.conn = pyodbc.connect(
                    "Driver={SQL Server};Server=DESKTOP-5OF5NG5\SQL; Database=APEL; uid=sa ; pwd=*;"
                )
            ...
            if (
                ...
                if self.write_db:
                    try:
                        ...
                        error_msg = (
                            f"{obj_id},{obj_time},{obj_lane},DB_WRITING_FAILED\n"
                        )
                        self.dberror_filewriter.write(error_msg)
                        print(error_msg)
    ...
    def _daily_filewriter_plotter(self, output_path):
        ...
        if status:
            print(f"\n error in function _daily_filewriter_plotter ! \n")
    ...
    def _hourly_plotter(self, output_path):
        ...
        if status:
            print(f"\n error in function _hourly_plotter ! \n")
    ...
    def _delete_oneday_videos(self, output_path):
        ...
        if status:
            print(f"\n error in function _delete_oneday_videos ! \n")
    ...
    def _clean_exit(self, currenthour_dir, currentday_dir):
        ...
        # import joblib
        # joblib.dump(self.tracker.civ, "civ_lane.pkl")
    ...
    def run(self):
        ...
        if self.write_db:
            dberror_filename = currentday_dir + date[:10] + f"_dberror.txt"
            self.dberror_filewriter = open(dberror_filename, "w")
        ...
        while self.vidcap.isOpened():
            ...
            frame_count += 1
            self.detector.frame_count = frame_count
            self.tracker.frame_count = frame_count
            ...
            if self.mode == "debug":
                ...
                cv2.line(frame, (pt[0], frame.shape[0]), (pt[0], 0), (255, 255, 255), 2)
                cv2.line(frame, (pt[1], frame.shape[0]), (pt[1], 0), (255, 255, 255), 2)
            ...
            if self.mode == "debug":
                det_text = "Vanilla Yolov4"
                if self.inference_type == "trt":
                    det_text = "Yolov4 on Tensorrt"
            ...
            for (k1, v1), (_, v2) in zip(
                ...
                for name, xcoord, bg in zip(
                    [Abbrevation2Name_Mapper[k1], str(v1), str(v2), str(v1 + v2)],
                    [15, 175, 275, 375],
            ...
            endtime = time.time()
            curr_fps = round(1.0 / (endtime - tik2), 4)
            avg_fps = round(frame_count / (endtime - tik1), 8)

            draw_text_with_backgroud(
                self.img_for_log,
                f"Instant fps: {curr_fps}",
                x=15,
                y=480,
                font_scale=0.5,
                thickness=1,
            ...
            cv2.imshow(f"ATCC, {self.mode} mode", out_frame)
